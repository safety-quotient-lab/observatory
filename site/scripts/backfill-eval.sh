...
#
# Environment:
#   DELAY    — seconds between launches (default: 2)
#   STATUS   — eval_status to query (default: pending)
#   PARALLEL — max concurrent evaluations (default: 3)
...
# Temp dir for per-worker output and results
WORK_DIR=$(mktemp -d)
trap 'rm -rf "$WORK_DIR"' EXIT

# Helper: execute SQL from a temp file
run_sql() {
  local sql_file
...
# Helper: execute D1 command silently (no output)
run_d1_silent() {
  npx wrangler d1 execute hrcb-db --remote --command "$1" 2>/dev/null >/dev/null || true
}

# Helper: format milliseconds as human-readable
fmt_ms() {
  local ms=$1
  if [ "$ms" -lt 1000 ]; then
    echo "${ms}ms"
...
# ── Worker function: evaluates a single story ──
# Buffers all output to $WORK_DIR/<hn_id>.log
# Writes metrics to $WORK_DIR/<hn_id>.result
eval_story() {
  ...
  # Mark as evaluating
  run_d1_silent "UPDATE stories SET eval_status = 'evaluating' WHERE hn_id = $HN_ID AND eval_status = '$STATUS'"
...
echo "╚══════════════════════════════════════╝"
echo "  Limit: $LIMIT | Delay: ${DELAY}s | Status: $STATUS | Parallel: $PARALLEL"
echo ""
...
# Collect hn_ids in order for printing results later
HN_IDS=()
...
# ── Aggregate timing from result files ──
SUCCESS=0
FAILED=0
TOTAL_FETCH_MS=0
TOTAL_DCP_MS=0

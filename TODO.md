...
- [ ] **Locate blog source + deploy HIGH novelty posts** — blog.unratified.org deployed via `unratified-blog` CF Pages (direct upload). Source not found locally. Locate it, then deploy: `byzantine-fault-tolerance-ui.md` + `confidence-weighting-inversion.md`. Personal notes + author review before publication.
...
- [ ] **Byzantine fault tolerance post** — full draft at `.claude/plans/exports/blog/byzantine-fault-tolerance-ui.md`. AUTO-PUBLISH (HIGH novelty). Blocked on: locating blog source.
- [ ] **Confidence weighting inversion post** — full draft at `.claude/plans/exports/blog/confidence-weighting-inversion.md`. AUTO-PUBLISH (HIGH novelty). Blocked on: locating blog source.

...
- [ ] **Write gap-detection blog post** — concrete demonstration: adding CF Web Analytics beacon without updating CSP in `middleware.ts` would have silently blocked all data collection. Gap-detection step in `/cycle` skill caught it in the same session before production traffic was affected. Angle: proactive gap-detection as a structural quality check — not a review of what you did, but a forced question about what you *didn't* examine. Pairs with the cogarch post (shows the skill system working in practice, not just in theory). Draft scaffold at `.claude/plans/exports/blog/gap-detection.md`.

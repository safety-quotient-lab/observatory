...
- `site/src/lib/db-stories.ts` — Story types, feed queries, dashboard stats, queue/failed stories, `getStory()` (scores+rater_scores fallback), `getFairWitnessForStory()` (fair_witness+rater_witness fallback)
...
- **Dual primary/rater table pattern**: `scores` + `fair_witness` store primary model results; `rater_scores` + `rater_witness` store per-model results. `getStory()` queries `scores` first; if empty (non-primary model eval), falls back to `rater_scores` for the eval_model (or best full-mode rater_eval if `stories.eval_model` is null). Same fallback pattern in `getFairWitnessForStory()` for `rater_witness`.
- **Item page uses materialized columns, not hcb_json**: The item page (`/item/[id]`) reads supplementary signals, labels, and metadata from materialized `stories` columns. Aggregates computed from scores via `computeAggregates()`. DCP from `domain_dcp` table. Fair Witness from `fair_witness`/`rater_witness` tables. `hcb_json` excluded from `getStory()` queries (~12-15KB savings per load).

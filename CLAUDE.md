...
- **Dual primary/rater table pattern**: `scores` + `fair_witness` store primary model results; `rater_scores` + `rater_witness` store per-model results. Both pairs are actively used — primary tables power the main feed/item pages, rater tables power multi-model comparison on `/item/[id]` and `/status/models`. This is intentional architecture, not duplication.
- **Supplementary signal sub-fields**: The 18 sub-field columns on `stories` (eq_source_quality, eq_evidence_reasoning, td_author_identified, so_framing, etc.) are materialized from the eval JSON for future SQL-side filtering/aggregation. Display always comes from `hcb_json` blob parsing. The columns are NOT orphaned — they're an intentional query-optimization layer.

...
- `scripts/hn-hrcb-daemon` — Continuous evaluator wrapper: auto-launches tmux session `hrcb-daemon`, loops `hn-hrcb-evaluate --pending 3 --parallel 3` with 5s sleep. Stop: `touch site/.daemon-stop`. Circuit breaker: 5 consecutive errors → auto-stop. Self-heals stuck `evaluating` stories (>10 min) on startup and every 20 batches. Log rotation at 5MB. 20-min subprocess timeout. Rate limit pause: 30 min sleep if `.rate_limited` sentinel detected.

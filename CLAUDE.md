...
- `site/src/lib/eval-write.ts` — D1 write functions (eval results, DCP cache). All full evals go through `writeEvalResult()` (writes to both legacy `scores`/`fair_witness` and `rater_scores`/`rater_witness` — no primary/non-primary branching). `updateConsensusScore()` called at end of both write paths — computes weighted mean across all done rater_evals (full=1.0 weight, light=0.5), updates stories.consensus_score/count/spread. `requestArchive(db, kv, hnId, url)` — KV-rate-limited (10s TTL) fire-and-forget Wayback Machine preservation, stores memento URL in stories.archive_url. `writeLightRaterEvalResult()` does COALESCE fill-in UPDATE to stories after writing rater_evals (fills eq_score, so_score, td_score, et_primary_tone, et_valence, et_arousal where null), then calls `refreshDomainAggregate`.
...
- **Unified read path via rater tables**: All read queries use `rater_scores`/`rater_witness` exclusively. Single-story queries JOIN `sc.eval_model = s.eval_model` to get the story's evaluating model's scores. Aggregate queries (SETL, fingerprints, domain stats) aggregate across all models with no model filter. Legacy `scores`/`fair_witness` tables still receive writes (for rollback safety) but are no longer read. `model_registry.is_primary` (migration 0045) makes primary model designation a DB flag. `getPrimaryModelId(db)` in models.ts queries it, falling back to `PRIMARY_MODEL_ID` constant.

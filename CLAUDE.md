...
- **Status** (`/status`): pipeline health glance — Coverage Spectrum (Multi-Model/Full/Lite/No Coverage funnel), Workers Health, Queue Breakdown (per-queue avg/d rate), Eval Velocity (stacked bar: full+lite evals/day over 60 days), Operations. Hub cards link to sub-pages. Sub-pages: `/status/models` (model registry + performance + measurement integrity — Evaluation Models, Channel Averages (HRCB/E/S triple bar per model), Multi-Model Raters, Evaluator Trust Index, Calibration, Signal Completeness, Model Drift, Content Type Validation, Score Distribution, Multi-Model Comparison, Section Divergences), `/status/events` (activity log + diagnostics — Pipeline Events, Recent Failures, Evaluation Queue, DLQ Details, API & Rate Limits, Cycle Performance, Throttle Impact, Eval Latency). All three pages use `cachedQuery` with KV (keys `sys:*`, TTLs 60-600s); ops-critical data stays uncached.
...
- `site/src/lib/eval-write.ts` — D1 write functions (eval results, DCP cache). `writeEvalResult()` updates the `stories` table only (legacy `scores`/`fair_witness` tables dropped — migration 0047). `writeRaterEvalResult()` writes to `rater_evals` + `rater_scores` + `rater_witness` + `eval_history`, then calls `writeEvalResult()` for story promotion. `updateConsensusScore()` called at end of both write paths — computes weighted mean across all done rater_evals (full=1.0 weight, lite=0.5), updates stories.consensus_score/count/spread. `requestArchive(db, kv, hnId, url)` — KV-rate-limited (10s TTL) fire-and-forget Wayback Machine preservation, stores memento URL in stories.archive_url. `writeLiteRaterEvalResult()` does COALESCE fill-in UPDATE to stories after writing rater_evals (fills eq_score, so_score, td_score, et_primary_tone, et_valence, et_arousal where null), then calls `refreshDomainAggregate`. `PT_TECHNIQUE_WEIGHTS` + `computePtScore()` — technique-inherent weights (Tier A=3: bypasses reasoning; B=2: manipulative framing; C=1: ubiquitous rhetoric); cumulative across all flag instances; NULL=not measured (lite), 0=measured+clean (full). `backfillPtScores()` — reads `pt_flags_json`, computes `pt_score`, writes to `stories` (WHERE pt_score IS NULL guard; also run `UPDATE stories SET pt_score=0 WHERE pt_flag_count=0 AND pt_score IS NULL` manually after). `PropagandaFlag.severity` was removed — severity is not part of PTC-18 rubric (Da San Martino et al. 2019).
...
- `site/src/lib/calibration.ts` — Full-model `CALIBRATION_SET` (hn_ids -1001..-1015) + lite-model `LITE_CALIBRATION_SET` (hn_ids -2001..-2015) + per-model thresholds + parameterized `runCalibrationCheck(scores, calSet?, thresholds?)`
...
### Local scripts
- `scripts/hn-hrcb-evaluate` — CLI evaluator: fetches pending stories from D1 (gate_category IS NULL guard), evaluates with `claude -p`, posts to `/api/ingest`. Full-mode only. Flags: `--pending N`, `--failed N`, `--parallel N`, `--model MODEL`, `--domain D`, `--min-score N`, `--dry-run`, `--status`. Default model: `claude-haiku-4-5-20251001`. Emits `METRIC {...}` JSONL lines per eval for analytics. Uses `--tools "" --no-session-persistence` on `claude -p`. Content guard: skips stories < 200 chars or < 150 words (JS-rendered/nav-only pages). Per-eval 5-min timeout. Rate limit detection: resets story to pending + touches `.rate_limited` sentinel so daemon pauses 30 min. Failure tracking: all failure/skip paths write `rater_evals` rows (eval_status='failed'/'skipped') so failures appear in status page's per-provider "Errs 24h" column.
- `scripts/hn-hrcb-daemon` — Continuous evaluator wrapper: auto-launches tmux session `hrcb-daemon`, loops `hn-hrcb-evaluate --pending N --parallel N` with 5s sleep. **Adaptive parallelism** (default): parses evaluator stdout signals (ok/fail counts, Claude latency, rate limits, queue empty) and auto-tunes `--parallel` between batches — slow ramp-up (+1 on all-ok), fast retreat (/2 on rate limit, -1 on >50% failures or >180s latency), reset to initial on empty queue. Max = 3× initial (floor 5). `--fixed` flag disables adaptive mode. Dynamic batch timeout: `PARALLEL × 300 + 300`, capped at 1800s. Stop: `touch site/.daemon-stop`. Circuit breaker: 5 consecutive errors → auto-stop. Self-heals stuck `evaluating` stories (>10 min) on startup and every 20 batches. Log rotation at 5MB. Rate limit pause: 30 min sleep if `.rate_limited` sentinel detected. Monitor decisions: `grep "ADAPTIVE:" site/daemon.log`.
- `scripts/evaluate-standalone.mjs` — Fetch queue from `/api/queue`, evaluate with `claude -p`, post to `/api/ingest`. Modes: `--mode full` (default) or `--mode lite`. Spawns claude with `{ CLAUDECODE: undefined, ANTHROPIC_API_KEY: undefined }` — both must be unset or the subprocess either refuses (CLAUDECODE) or uses depleted API credits instead of the OAuth subscription (ANTHROPIC_API_KEY). Failure mode: exit 1, "Credit balance is too low" on stdout, empty stderr.
**Lite calibration workflow (server-side — canonical path):**
...
`INGEST_URL=... INGEST_SECRET=... node scripts/evaluate-standalone.mjs --mode lite --dry-run --url https://example.com --hn-id -9999`
...
- **Semantic color system**: `global.css` defines CSS custom properties in `:root` — core palette (`--bg-*`, `--fg-*`, `--color-*`), channel colors (`--channel-hrcb`, `--channel-editorial`, `--channel-structural`), status colors (`--color-negative`, `--color-warning`, `--color-positive` with light/medium variants), categorical (`--color-sentiment`, `--color-theme`). Badge utility classes (`.badge-positive`, `.badge-negative`, etc.) encode the `rgba(color, 0.15)` background + color pattern. Text shortcuts (`.text-positive`, `.text-muted`, etc.) for semantic text coloring. Dynamic score-derived colors use `scoreToColor()` HSL interpolation in `colors.ts` (not CSS vars). ~1,395 inline hex values remain in .astro files — migration to var() refs is incremental.
...
- **eval_status lifecycle**: `pending` → `queued` (dispatched to eval_queue) → `evaluating` (manual SSE eval via `/api/trigger/[id]`) → `done` | `failed` | `skipped` | `rescoring`. `evaluating` is a transient status set only by the manual trigger endpoint — daemon self-heals stuck `evaluating` stories (>10 min). `StatusCounts` interface has its own `evaluating` field. Feed/query filters group `pending`+`queued`+`evaluating` together. `markFailed`/`markSkipped` guards: `NOT IN ('done', 'rescoring')`.
- **domain_aggregates column names**: The table uses `avg_hrcb` (not `avg_hcb`), `evaluated_count` (not `eval_count`), `story_count` (total crawled), `avg_pt_count` (raw flag count average), `avg_pt_score` (weighted technique score average — migration 0053). When writing D1 queries against this table, use these names. `avg_hrcb` is null for domains where all evaluated stories had null `hcb_weighted_mean` (e.g. ghost stories) — self-corrects when stories re-evaluate.
...
- **Calibration IDs**: Full model: synthetic hn_ids -1001 to -1015 (`CALIBRATION_SET`). Lite model: -2001 to -2015 (`LITE_CALIBRATION_SET`). Lite cal enqueued via `POST /calibrate?mode=lite` (inserts as pending for local evaluator), checked via `POST /calibrate/check?mode=lite` (reads `rater_evals` with `prompt_mode='lite'`). Both sets shown on `/status/models` (Calibration cards).
- **DCP caching**: 7-day TTL in KV per domain, also persisted to `domain_dcp` table in D1.
- **Lite prompt mode**: Small/free models (Workers AI: `llama-4-scout-wai` + `llama-3.3-70b-wai` — both on `WORKERS_AI_QUEUE`; `llama-3.3-70b` on OpenRouter was disabled due to chronic 429s) use `METHODOLOGY_SYSTEM_PROMPT_LITE` — editorial-only single score + 5 supplementary signals (eq, so, td, valence, arousal) + primary_tone (~200-400 output tokens vs ~4-5K for full). Schema `lite-1.4` (was `lite-1.3`). **lite-1.4 changes**: integer 0-100 scoring (50=neutral) instead of float [-1,+1] — eliminates bimodal 0.0/0.8 clustering; explicit tier anchors (90-100=NGO advocacy, 50=utility, 0-10=propaganda); `reasoning` field (max 10 words, placed before `editorial` in JSON template so model pre-commits before outputting digit; stored in `rater_evals.reasoning` — migration 0048 — for post-hoc diagnosis of lazy vs genuine neutral-50 scoring). Parser detects v1.4 by `schema_version === 'lite-1.4'` (or legacy `'light-1.4'`) or value > 1.0, converts via `(score-50)/50`. Old lite-1.3 scores cleared by migration 0046 (applied 2026-02-27 — 1,456 lite-only stories reset to pending for re-evaluation). Field `executive_summary` renamed to `short_description` (max 20 words). Controlled by `ModelDefinition.prompt_mode: 'full' | 'lite'`. No structural channel, no per-section scores, no DCP, no Fair Witness evidence, no SR, no PT (pt_flag_count = null, pt_score = null — not measured in lite). Results written to `rater_evals` with `prompt_mode = 'lite'` (no `rater_scores`/`rater_witness`). After each lite rater_evals write, `writeLiteRaterEvalResult` does a COALESCE fill-in UPDATE to `stories` — fills eq/so/td/tone/valence/arousal + hcb_editorial_mean + theme/sentiment tags (nulls only). Lite evals do NOT promote `eval_status` — stories remain pending/queued until a full eval runs. Lite dispatch in `enqueueForEvaluation` sends ALL pending stories to Workers AI queue so they get fast ~lite editorial scores while awaiting full eval. Stuck-queued recovery in `enqueueForEvaluation` resets stranded `queued` stories (with no full eval) back to `pending` when Anthropic becomes available. `EvalCard.astro`: `hasEval` checks `hcb_weighted_mean !== null || hcb_editorial_mean !== null` (no `eval_status` check — shows scores even for pending stories with lite evals); `isLiteOnly = hcb_weighted_mean === null || hcb_structural_mean === null` suppresses S: channel display (also catches malformed full evals that returned editorial-only); lite evals show boxed `[L]` icon instead of `~lite` text. `displayScore = hcb_weighted_mean ?? hcb_editorial_mean` (weighted first — was incorrectly reversed before 2026-02-28).
...
- **QueueMessage `prompt_mode`**: Non-primary model queue messages now include `prompt_mode: model.prompt_mode` (set at dispatch time in `hn-bot.ts` and `cron.ts`). Consumer uses this as fallback in `isLiteMode` detection (alongside model registry lookup).
...
- **eval-write FK guards**: All 3 write functions (`writeEvalResult`, `writeRaterEvalResult`, `writeLiteRaterEvalResult`) do a `SELECT 1 FROM stories WHERE hn_id = ?` guard at entry — throws if story doesn't exist (stale queue message), preventing orphaned eval rows.
- **Structural channel guard**: `writeRaterEvalResult` checks if a full eval has `hcb_structural_mean === null` (editorial-only). If so, the rater data is still written (editorial data preserved) but the story is NOT promoted to `done` — it stays pending for re-evaluation. Logs an `eval_skip` event. Prevents misleading composite scores when a model returns a malformed partial response.
...
- **Credit pause fallback**: When `credit_pause:anthropic` KV key is set, `enqueueForEvaluation` skips the Anthropic queue and dispatches to free model queues (Workers AI + free OpenRouter) instead. Lite queue always fires regardless of credit state. `env` object passed through `runCrawlCycle` → `enqueueForEvaluation` to enable free-model fallback queue refs.
- **Feed filter/sort COALESCE**: `db-stories.ts` positive/negative/neutral filters and score_desc/asc sorts use `COALESCE(hcb_weighted_mean, hcb_editorial_mean)` so lite-only stories surface correctly. Both score columns are semantically compatible for ordering (both editorial scale [-1,1]).

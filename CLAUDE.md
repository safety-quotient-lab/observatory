...
- `scripts/gemini-feedback.mjs` — Interactive REPL for multi-turn Gemini feedback exchanges. Sends site context (agent-card, manifest, architecture summary), receives structured JSON feedback, fact-checks against known truths. Commands: `.save`, `.context`, `.eval`, `.model`, `.quit`. Flags: `--model MODEL`, `--dry-run`, `--resume FILE`. Saves to `.claude/plans/memorized/gemini-exchanges/`. Requires `GOOGLE_API_KEY` in `site/.dev.vars`.

...
- **Light calibration cloud vs standalone gap**: `POST /calibrate?mode=light` + cloud consumer tests actual production models (llama-4-scout-wai, llama-3.3-70b-wai). `evaluate-standalone.mjs --mode light` uses `claude -p claude-haiku-4-5` locally (validates prompt structure, not production models). Cloud consumer IPs (Cloudflare Workers egress) can get blocked by some cal targets — booking.com (EX-3, hn_id=-2013) returns 157-char bot-protection page to Workers IPs, triggering `age_gate` content gate. Standalone evaluator (local IP) gets the real page. EX-3 failing in cloud cal runs = content gate false positive, not model failure.

...
- `site/functions/sweeps.ts` — 23 sweep handlers (``). Add new sweeps here + one entry in `SWEEP_HANDLERS` in `cron.ts`. Kagi sweeps have KV-backed rate limit backoff (`kagi:backoff` key, 30-min TTL).
